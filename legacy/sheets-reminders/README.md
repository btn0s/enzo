# Enzo legacy tracker: a tiny system for very long nights
...
The Urgent switch is not available through AppleScript. AppleScript owns the reminder date, while a small native Swift Accessibility helper enables and verifies Urgent against the Reminders interface. It may need adjustment after major macOS UI changes. Sheet logging remains independent; if the reminder step fails after a feed, the CLI prints a safe repair command instead of writing the event twice.
...
| `ENZO_REMINDER_ATTEMPTS` | No | `3` |
| `ENZO_REMINDER_TIMEOUT_SECONDS` | No | `30` |
